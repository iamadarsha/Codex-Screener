...
|---|---|---|---|---|
| **Upstox V3 WebSocket feed** | Primary realtime tick data (target) | Bearer token (Analytics Token recommended) | Binary Protobuf. `sub`/`unsub`/`change_mode`. Subscription caps: `ltpc` 5000 keys, `full` 2000, `option_greeks` 3000, `full_d30` 50 (Upstox Plus only). First message on connect is `market_info` (segment status + Closing Auction Session data), then a snapshot, then live ticks. | ❌ Not implemented — repo currently targets the **V2** endpoint (`wss://api.upstox.com/v2/feed/market-data-feed`). Phase 2 work. |
| **Upstox Analytics Token** | Recommended auth for the owner-controlled backend | Generated from Developer Apps page, no OAuth redirect | Read-only, 1-year validity, one active token per account. Covers market quotes, historical data, option chains, fundamentals, news, websocket (no static IP needed for these). Free. | ❌ Not used — repo uses standard OAuth2 authorization-code flow (`upstox_auth.py`), which has no refresh-token support and forces re-login when the token expires. Recommend switching in Phase 2. |
